"""
Train deterministic SAC with reward scale sweep using vectorized environments (faster).
"""
import os
import time
import yaml
...
from sac_deterministic.agent import DeterministicSACAgent, DetSACConfig


def evaluate(env, agent: DeterministicSACAgent, episodes: int, seed: int) -> float:
...
def train_one_vec(
    ...
    agent = DeterministicSACAgent(obs_dim, act_dim, agent_cfg)
...
def main():
    ...
    run_dir = os.path.join("runs", f"{env_id}_DetSAC_vec_{int(time.time())}")
    os.makedirs(run_dir, exist_ok=True)
    
    print("="*70)
    print(f"DetSAC Reward Scale Sweep - {env_id}")
    print("="*70)
